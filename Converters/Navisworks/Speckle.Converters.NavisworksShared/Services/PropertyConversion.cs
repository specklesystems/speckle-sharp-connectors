...
public class PropertyConverter(IUiUnitsCache uiUnitsCache) : IPropertyConverter
{
  ...
  public object? ConvertPropertyValue(NAV.VariantData? value, NAV.Units modelUnits, string propDisplayName) =>
    value == null ? null
    : _handlers.TryGetValue(value.DataType, out var f) ? f(value, (modelUnits, propDisplayName))
    : value.DataType is NAV.VariantDataType.None or NAV.VariantDataType.Point2D ? null
    : value.ToString();
