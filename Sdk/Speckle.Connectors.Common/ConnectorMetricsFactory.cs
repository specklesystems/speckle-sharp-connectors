...
public sealed class ConnectorMetricsFactory : ISdkMetricsFactory, IDisposable
{
  ...
  public ISdkCounter<T> CreateCounter<T>(string name, string? unit = default, string? description = default) where T : struct 
    => new ConnectorCounter<T>(_loggingMetricsFactory.CreateCounter<T>(name, unit, description));
    ...
    where T : struct
  {
    ...
    public void Add(T value, KeyValuePair<string, object?> tag1, KeyValuePair<string, object?> tag2) => counter.Add(value, tag1, tag2);
    public void Add(
      T value,
