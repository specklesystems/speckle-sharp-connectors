...
/// On .NET Framework that resolves against the process (<c>Revit.exe</c>) directory — NOT the addin folder — so the
...
/// session would otherwise hit "No compression codec for Zstd is available". net8+ (Revit 2025+) resolves the native
/// via its RID, so this is a no-op there.
/// </remarks>
internal static class RevitZstdNativeLoader
{
#if NETFRAMEWORK
  ...
  public static void Ensure(ILogger? logger = null)
  {
    ...
    try
    {
      var dir = Path.GetDirectoryName(typeof(RevitZstdNativeLoader).Assembly.Location);
