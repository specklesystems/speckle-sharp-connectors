...
/// <summary>
/// Shared machinery for the asynchronous Publish components. Subclasses differ only in whether they write a 4.0
/// bundle (<see cref="UseArtifacts"/>) and whether they take model-wide properties
/// (<see cref="HasModelPropertiesInput"/>).
/// </summary>
public abstract class SendAsyncComponentBase : GH_AsyncComponent<SendAsyncComponentBase>
{
  protected SendAsyncComponentBase(
    string name,
    string nickname,
    string description,
    string category,
  ...
  public abstract bool UseArtifacts { get; }

  /// <summary>Whether this variant exposes the model-wide properties input. It is the last input, so no other index moves.</summary>
  protected abstract bool HasModelPropertiesInput { get; }

  /// <summary>Hook for the deprecated variant to say what it created. No-op otherwise.</summary>
  ...
  private bool ParseInput(IGH_DataAccess da, IServiceScope scope)
  {
    ...
    else
    {
      ...
      var docName = SendComponentBase.GetGrasshopperFileInfo().fileName ?? "Unnamed Document";
      if (
...
public class SendComponentWorker : WorkerInstance<SendAsyncComponentBase>
{
  public SendComponentWorker(
    SendAsyncComponentBase p,
  ...
  public override WorkerInstance<SendAsyncComponentBase> Duplicate(string id, CancellationToken cancellationToken)
  {
  ...
  public override void SetData(IGH_DataAccess da)
  {
    ...
    if (!hasWarnings)
    {
      ...
      Parent.AddRuntimeMessage(
        ...
        $"Successfully published {((SendAsyncComponentBase)Parent).RootCollectionWrapper?.Value.Collection.GetTotalChildrenCount()} objects to Speckle."
      );
      */
      Parent.AddRuntimeMessage(
        GH_RuntimeMessageLevel.Remark,
  ...
  private async Task Send(Action<string, double> reportProgress)
  {
    ...
    var (fileName, fileBytes) = SendComponentBase.GetGrasshopperFileInfo();
...
public class SendAsyncComponentAttributes : GH_ComponentAttributes
{
  ...
  protected override void Render(GH_Canvas canvas, Graphics graphics, GH_CanvasChannel channel)
  {
    ...
    var state = ((SendAsyncComponentBase)Owner).CurrentComponentState;

    if (channel == GH_CanvasChannel.Objects)
    {
      if (((SendAsyncComponentBase)Owner).AutoSend)
      {
  ...
  public override GH_ObjectResponse RespondToMouseDown(GH_Canvas sender, GH_CanvasMouseEvent e)
  {
    if (e.Button == MouseButtons.Left)
    {
      if (((RectangleF)ButtonBounds).Contains(e.CanvasLocation))
      {
        ...
        if ((Owner as SendAsyncComponentBase)?.HasMultipleInputs == true)
        {
          return GH_ObjectResponse.Handled;
        }
        if (((SendAsyncComponentBase)Owner).AutoSend)
        {
          ((SendAsyncComponentBase)Owner).AutoSend = false;
          Owner.OnDisplayExpired(true);
          return GH_ObjectResponse.Handled;
        }
        if (((SendAsyncComponentBase)Owner).CurrentComponentState == ComponentState.Sending)
        {
          return GH_ObjectResponse.Handled;
        }
        ((SendAsyncComponentBase)Owner).CurrentComponentState = ComponentState.Ready;
